...
# ══════════════════════════════════════════════════════
@st.cache_data(ttl=1) # Fuerza la actualización cada segundo para ver cambios de Jupyter
...
if st.session_state.screen == 'splash':
    ...
    # --- GRÁFICA Y EXPLICACIÓN DE DATOS DUROS ---
    ...
    with cl:
        ...
        </script>""".replace("__BPD__", "350").replace("__MEJ__", str(d['mejora']))
        components.html(HTML_CHART, height=530)
        
    with cr:
        st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)
        # 🧠 SECCIÓN TÉCNICA EXPLICATIVA CON unsafe_allow_html=True 🧠
        st.markdown(f"""
        <div style="background:#0D1520; padding:25px; border-radius:12px; border:1px solid rgba(0,229,160,0.3); height:500px; overflow-y:auto;">
            ...
            <div class="tech-box">
                ...
                <p class="reasoning-text"><b>¿Por qué importa?</b> Una viscosidad alta garantiza que el fluido empuje el crudo sin "dedear" el agua, maximizando el barrido areal[cite: 3, 4].</p>
            </div>
            
            <div class="tech-box">
                <p class="tech-label">YIELD POINT (YP): {d.get('yield_p', 'N/A')} lb/100ft²</p>
                <p class="reasoning-text"><b>¿Por qué importa?</b> Indica la estabilidad bajo presión. Un YP balanceado evita la degradación mecánica del polímero en el fondo[cite: 3].</p>
            </div>
            
            <div class="tech-box">
                <p class="tech-label">PAYBACK: {d['payback']} MESES</p>
                <p class="reasoning-text"><b>¿Por qué importa?</b> Retorno casi inmediato. El crudo extra cubre el costo tecnológico en tiempo récord[cite: 3, 6].</p>
            </div>
