...
def generate_pdf_base64(d):
    ...
    pdf.cell(50, 8, "Skin Damage", 1); pdf.cell(70, 8, "Alto Riesgo", 1); pdf.cell(70, 8, "Minimo", 1, 1)

    binary_pdf = pdf.output(dest="S").encode("latin-1")
    return base64.b64encode(binary_pdf).decode()

# ══════════════════════════════════════════════════════
# 4. MOTOR DE IA (SEGURO EN SERVIDOR)
# ══════════════════════════════════════════════════════
def get_ai_analysis(f, t, p, b):
    url = "https://api.groq.com/openai/v1/chat/completions"
    headers = {"Authorization": f"Bearer {GROQ_API_KEY}", "Content-Type": "application/json"}
    prompt = f"Act as EOR physicist. JSON only. Fluid={f}, Tubing={t}, Wells={p}, BPD={b}. Return: mejora, ahorro_usd, fee_usd, co2_tons, eur_bbls, mpy."
    try:
...
if st.session_state.screen == 'splash':
    ...
    with k1: st.markdown(f'<div class="kpi-box"><p class="kpi-label">AHORRO OPEX</p><p class="kpi-value" style="color:#00E5A0;">${d["ahorro"]:,.0f}</p></div>', unsafe_allow_html=True)
    ...
    with col_r:
        ...
        st.markdown(f'<br><a href="data:application/pdf;base64,{pdf_b64}" download="FlowBio_Report.pdf" style="text-decoration:none;"><button style="background:#00E5A0; border:none; padding:15px; border-radius:8px; width:100%; color:#060B11; font-weight:800; cursor:pointer;">📥 REPORTE OFICIAL</button></a>', unsafe_allow_html=True)
