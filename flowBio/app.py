...
def demo_data():
    return {
        "Well-UKCS-Alpha": {
            ...
            "quimico":"Goma Xantana","ppm":1760,"vol_pv":0.31,"bwpd":478,
...
# ══════════════════════════════════════════════════════
# 3. PDF CORPORATIVO (FIX DEFINITIVO FPDF)
# ══════════════════════════════════════════════════════
def clean_text(text):
    """Limpia caracteres que FPDF no puede codificar en latin-1"""
    if text is None: return ""
    text = str(text)
    # Reemplazar símbolos problemáticos específicos
    text = text.replace('·', '.').replace('²', '2').replace('—', '-')
    # Forzar codificación estricta (reemplaza cualquier otro símbolo raro por '?')
...
def generate_pdf(well, d):
    ...
    pdf.cell(0,15, clean_text(f"FlowBio Executive Report: {well}"), 0, 1)
    
    pdf.set_font('Arial','',11)
    pdf.set_text_color(0,229,160)
    pdf.cell(0,8, clean_text(f"Generado: {datetime.now().strftime('%Y-%m-%d %H:%M')} | PIML v0.3"), 0, 1)
    pdf.ln(6)
    
    pdf.set_fill_color(13,21,32); pdf.set_text_color(255,255,255)
    
    # Se pasan los strings crudos, clean_text los limpia justo al insertarlos
    rows_pdf = [
        ["Barriles Extra/Dia",  f"{d.get('extra_bpd',0):.1f} bbl/dia"],
        ...
        ["PV (Visc. Plastica)", f"{d.get('visc_p',0)} mPa.s"],  # Cambiado · por . por seguridad
        ["YP (Yield Point)",    f"{d.get('yield_p',0)} lb/ft2"], # Cambiado ² por 2 por seguridad
        ["EUR Acumulado",       f"{d.get('eur',0):,} bbls"],
        ["Quimico EOR",         str(d.get('quimico','-'))],
        ["Concentracion",       f"{d.get('ppm',0)} ppm"],
        ["Factor Skin",         f"{d.get('skin',0):.2f}"],
        ["Recomendacion",       str(d.get('recomendacion','-'))],
    ]
    
    for k,v in rows_pdf:
        pdf.cell(95,11, clean_text(f" {k}"), 1, 0, 'L', True)
        pdf.cell(95,11, clean_text(f" {v}"), 1, 1, 'R')
        
    pdf.ln(8)
    pdf.set_text_color(100,116,139)
    pdf.set_font('Arial','I',9)
    pdf.cell(0,8, clean_text("FlowBio Intelligence . PIML v0.3 . TRL 4 . Orizaba, Veracruz, MX"), 0, 1, 'C')
...
with k1:
    st.markdown(f"""<div class="kpi-box"><p class="kpi-label">CRUDO INCREMENTAL / MES</p><p class="kpi-value">+{extra_mes:,} <span style="font-size:16px">bbls</span></p><p class="kpi-desc">{extra_bpd:.1f} bbl/día</p></div>""", unsafe_allow_html=True)
with k2:
    st.markdown(f"""<div class="kpi-box cyan"><p class="kpi-label">VALOR EXTRA GENERADO</p><p class="kpi-value" style="color:#22D3EE">${valor_extra:,}</p><p class="kpi-desc">A precio actual</p></div>""", unsafe_allow_html=True)
with k3:
    st.markdown(f"""<div class="kpi-box amber"><p class="kpi-label">SUCCESS FEE MENSUAL</p><p class="kpi-value" style="color:#F59E0B">${fee_mensual:,}</p><p class="kpi-desc">Modelo FlowBio</p></div>""", unsafe_allow_html=True)
with k4:
    color = "#EF4444" if payback_val > 6 else "#00E5A0"
    st.markdown(f"""<div class="kpi-box {'red' if payback_val > 6 else ''}"><p class="kpi-label">PAYBACK</p><p class="kpi-value" style="color:{color}">{payback_val} <span style="font-size:16px">meses</span></p></div>""", unsafe_allow_html=True)
...
with cl:
    ...
    var t1={{x:x,y:y_base,name:'Status Quo',type:'scatter',line:{{color:'#EF4444',dash:'dot',width:2}}}};
    var t2={{x:x,y:y_flow,name:'FlowBio EOR',type:'scatter',line:{{color:'#00E5A0',width:3}},fill:'tonexty',fillcolor:'rgba(0,229,160,0.08)'}};
    var layout={{paper_bgcolor:'transparent',plot_bgcolor:'transparent',font:{{color:'#64748B',family:'DM Mono',size:10}},margin:{{t:20,b:40,l:55,r:20}},xaxis:{{gridcolor:'#152335'}},yaxis:{{gridcolor:'#152335'}},showlegend:true,legend:{{bgcolor:'rgba(0,0,0,0)',font:{{color:'#8BA8C0'}}}}}};
...
with cr:
    ...
    rec_col = "#00E5A0" if "INYECTAR" in rec else "#F59E0B" if "EVALUAR" in rec else "#64748B"
    skin_v  = d.get("skin",0)
    skin_col= "#EF4444" if skin_v>8 else "#F59E0B" if skin_v>3 else "#00E5A0"

    html_ins = f"""
    <div style='background:#0D1520;padding:24px;border-radius:12px;border:1px solid rgba(0,229,160,0.2);height:415px;overflow-y:auto'>
      <p style='color:#00E5A0;font-family:DM Mono;font-size:11px;letter-spacing:1.5px;margin-bottom:16px'>🧠 ENGINEERING INSIGHTS</p>
      <p style='font-family:DM Mono;font-size:13px;color:#22D3EE;margin-bottom:2px'>QUÍMICO: {d.get('quimico','-')}</p>
      <p style='font-size:11px;color:#8BA8C0;margin-top:0;margin-bottom:12px'>Concentración: {d.get('ppm',0)} ppm . PV: {d.get('vol_pv',0)}</p>
      <p style='font-family:DM Mono;font-size:13px;color:#22D3EE;margin-bottom:2px'>CAUDAL: {d.get('bwpd',0)} bwpd</p>
      <p style='font-size:11px;color:#8BA8C0;margin-top:0;margin-bottom:12px'>Límite de fractura: max {d.get('lim_psi',0):,} psi</p>
      <p style='font-family:DM Mono;font-size:13px;color:#22D3EE;margin-bottom:2px'>WC REDUCTION: {d.get('wc_red',0):.1f}%</p>
      <hr style='opacity:0.1;margin:14px 0'>
      <p style='color:#64748B;font-family:DM Mono;font-size:10px;letter-spacing:1px'>FACTOR SKIN:</p>
      <p style='color:{skin_col};font-family:Syne;font-size:28px;font-weight:800;margin:2px 0'>S = {skin_v:.2f}</p>
      <p style='color:#64748B;font-family:DM Mono;font-size:10px;letter-spacing:1px;margin-top:12px'>RECOMENDACIÓN PIML:</p>
      <p style='color:{rec_col};font-family:DM Mono;font-size:13px;font-weight:500'>{rec}</p>
      <hr style='opacity:0.1;margin:14px 0'>
      <p style='color:#64748B;font-family:DM Mono;font-size:10px'>EUR ACUMULADO (5 años):</p>
      <p style='color:#00E5A0;font-family:Syne;font-size:26px;font-weight:800;margin:0'>{d.get('eur',0):,} <span style='font-size:13px;color:#64748B'>bbls</span></p>
    </div>"""
