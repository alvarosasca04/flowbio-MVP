...
# ══════════════════════════════════════════════════════
# 2. MOTOR DE REPORTE PDF (RÉPLICA EXACTA DEL OFICIAL)
# ══════════════════════════════════════════════════════
class FlowBioReport(FPDF):
    def header(self):
        # Encabezado corporativo oscuro
...
def generate_pdf_base64(d):
    ...
    pdf.cell(0, 5, f"ahorro anual estimado con Na-CMC FlowBio en {d['label']}", 0, 1)
    pdf.ln(5)
    ...
    pdf.set_xy(10, 105)
    pdf.chapter_title('ANÁLISIS REOLOGICO - MODELO POWER LAW (PIML)')
    pdf.draw_kpi_box("Indice flujo (n)", "0.569", 10, 118)
    pdf.draw_kpi_box("Consistencia K", "151.4", 58, 118)
    pdf.draw_kpi_box("Ratio movilidad", "0.28", 106, 118)
    pdf.draw_kpi_box("Ef. barrido", f"{d['mejora']:.1f}%", 154, 118)
    ...
    pdf.cell(50, 8, "Biodegradabilidad", 1); pdf.cell(70, 8, "No biodegradable", 1); pdf.cell(70, 8, "100% biodegradable", 1, 1)
    pdf.cell(50, 8, "Skin Damage", 1); pdf.cell(70, 8, "Alto Riesgo", 1); pdf.cell(70, 8, "Minimo (Bio-friendly)", 1, 1)
    pdf.cell(50, 8, "HSE Risk Factor", 1); pdf.cell(70, 8, "Toxico", 1); pdf.cell(70, 8, "Seguro", 1, 1)

    # 4. DIAGNÓSTICO
    pdf.ln(10)
    pdf.chapter_title('DIAGNOSTICO TECNICO MOTOR PIML')
    pdf.set_font('Arial', '', 10); pdf.set_text_color(50, 50, 50)
    txt = (f"El análisis de los {d['pozos']} pozos confirma comportamiento pseudoplástico adecuado. "
           f"La eficiencia de barrido proyectada de {d['mejora']:.1f}% representa un incremento sobre el baseline. "
           "Se recomienda el inicio de inyección basado en la estabilidad térmica molecular detectada.")
    pdf.multi_cell(0, 5, txt)

    return base64.b64encode(pdf.output(dest="S").encode("latin-1")).decode()

# ══════════════════════════════════════════════════════
# 3. MOTOR IA Y PANTALLAS
# ══════════════════════════════════════════════════════
def get_ai_analysis(f, t, p, b):
    url = "https://api.groq.com/openai/v1/chat/completions"
    prompt = f"Act as EOR eng. JSON only. Fluid={f}, Tubing={t}, Wells={p}, BPD={b}. Return: mejora, ahorro_usd, fee_usd, co2_tons, eur_bbls, mpy."
    try:
        r = requests.post(url, headers={"Authorization": f"Bearer {GROQ_API_KEY}"}, json={"model": "llama-3.3-70b-versatile", "messages": [{"role": "user", "content": prompt}], "response_format": {"type": "json_object"}, "temperature": 0.1}, timeout=15)
        ...
        return {
            ...
            "pozos": int(p), "bpd": float(b), "label": "IA LIVE PROJECTION"
        }
    except: return None
