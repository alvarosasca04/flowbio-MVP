...
<body class="min-h-screen">
    ...
    <div id="page-landing" class="h-screen w-full flex flex-col justify-center items-center p-10 text-center relative bg-[#080a0d]">
        ...
        <button onclick="goToStep1()" class="btn-deploy mt-10 px-14 py-5 rounded-xl tracking-widest text-xs"> INICIALIZAR INSTANCIA CLOUD </button>
    </div>

    <div id="step-1" class="hidden max-w-5xl mx-auto py-10 reveal">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="glass p-8">
                <label class="label-tech">Químico EOR Actual</label>
            ...
            <div class="glass p-8">
                <label class="label-tech">Infraestructura de Bombeo</label>
        ...
        <div class="glass p-8 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div><div class="flex justify-between text-[10px] mono mb-3 text-slate-400"><span>Permeabilidad (K)</span><span class="text-emerald-500">450 mD</span></div><input type="range" min="10" max="2000" value="450"></div>
        ...
        <div class="flex justify-center pt-8"><button onclick="runSimulation()" class="btn-deploy px-16 py-5 rounded-xl text-xs"> ⚡ Desplegar Agentes en AWS </button></div>
    </div>

    <div id="step-2" class="hidden max-w-3xl mx-auto py-20">
        <div class="terminal flex flex-col h-[420px] shadow-2xl overflow-hidden">
            <div id="terminal-content" class="p-8 flex-1 mono text-[13px] text-emerald-500/80 space-y-4 overflow-y-auto"></div>
        </div>
    </div>

    <div id="step-3" class="hidden space-y-6 p-4">
        <div class="glass p-8"><div id="main-plot" class="w-full h-96"></div></div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="glass p-8 border-l-4 border-emerald-500"><p class="text-[10px] uppercase text-slate-500 mb-2 font-bold">Crudo Extra</p><h3 class="text-5xl font-black text-emerald-500 tracking-tighter">+25,000 <span class="text-sm font-normal">bbls</span></h3></div>
            <div class="glass p-8 border-l-4 border-white"><p class="text-[10px] uppercase text-slate-500 mb-2 font-bold">NPV</p><h3 class="text-5xl font-black text-white tracking-tighter">$1.72M</h3></div>
            <div class="glass p-8 border-l-4 border-emerald-500 bg-emerald-500/5"><p class="text-[10px] uppercase text-emerald-500 mb-2 font-bold">Success Fee</p><h3 class="text-5xl font-black text-white tracking-tighter">$75,000</h3></div>
        </div>
    </div>

    <script>
        function goToStep1() { document.getElementById('page-landing').classList.add('hidden'); document.getElementById('step-1').classList.remove('hidden'); lucide.createIcons(); }
        async function runSimulation() {
            ...
            const lines = [
                {t: "AWS Cloud:", m: "Sincronizando metadatos...", s: "[DONE]"},
                {t: "🤖 Agente Física:", m: "Validando reología...", s: "[OK]"},
                {t: "🤖 Agente Financiero:", m: "Calculando ROI...", s: "[OK]"}
            ];
            for (let l of lines) {
            ...
            setTimeout(() => { document.getElementById('step-2').classList.add('hidden'); document.getElementById('step-3').classList.remove('hidden'); renderPlot(); lucide.createIcons(); }, 800);
        }
        function renderPlot() {
...
# 4. LANZAMIENTO (ESTO ES LO QUE CIERRA EL BLOQUE)
