...
# ══════════════════════════════════════════════════════
# 3. MOTOR DE REPORTE PDF (FORMATO OFICIAL)
# ══════════════════════════════════════════════════════
...
def generate_pdf_base64(d):
    ...
    pdf.set_font('Arial', 'B', 24); pdf.set_text_color(0, 150, 100)
    pdf.cell(0, 20, f"${d['ahorro']:,.0f} USD/año", 0, 1)
    
    pdf.ln(5); pdf.set_font('Arial', 'B', 12); pdf.set_text_color(0, 229, 160)
    pdf.cell(0, 10, 'ANÁLISIS REOLÓGICO - MODELO POWER LAW (PIML)', 0, 1)
    pdf.set_font('Arial', '', 10); pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 8, f"- Indice de flujo (n): 0.569", 0, 1)
    pdf.cell(0, 8, f"- Eficiencia de barrido proyectada: {d['mejora']:.1f}%", 0, 1)
    pdf.cell(0, 8, f"- Reservas EUR extra (5A): {d['eur']:,.0f} bbls", 0, 1)

    pdf.ln(10); pdf.set_font('Arial', 'B', 12); pdf.set_text_color(0, 229, 160)
    pdf.cell(0, 10, 'COMPARATIVA TECNICA', 0, 1)
    ...
    pdf.cell(60, 8, "Biodegradabilidad", 1); pdf.cell(65, 8, "No biodegradable", 1); pdf.cell(65, 8, "100% biodegradable", 1, 1)
    pdf.cell(60, 8, "Skin Damage", 1); pdf.cell(65, 8, "Alto Riesgo", 1); pdf.cell(65, 8, "Minimo", 1, 1)
...
def get_ai_analysis(fluido, tuberia, pozos, bpd):
    ...
    prompt = f"Act as EOR physicist. JSON only. Fluid={fluido}, Tubing={tuberia}, Wells={pozos}, BPD={bpd}. Return: mejora, ahorro_usd, fee_usd, co2_tons, eur_bbls, mpy, payback, wc_red."
    try:
...
if st.session_state.screen == 'splash':
    ...
    with c2:
        if st.button("DEMO REAL S3 (10 POZOS)"):
    ...
    with c3:
        if st.button("SIMULADOR IA (MANUAL)"):
            st.session_state.screen = 'setup'; st.rerun()

elif st.session_state.screen == 'setup':
    st.markdown("<div style='padding:60px 100px;'><h2 style='font-family:Syne; font-size:45px; color:white;'>Configuracion de Yacimiento</h2>", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        f = st.selectbox("QUÍMICO", ["Na-CMC (FlowBio Eco-Safe)", "HPAM (Tradicional)"])
        t = st.selectbox("TUBERÍA", ["Acero al Carbono (L-80)", "Aleacion CRA (13Cr)"])
    with c2:
        p = st.number_input("NÚMERO DE POZOS", value=15); b = st.number_input("BPD BASE POR POZO", value=350)
    if st.button("🧠 GENERAR ANÁLISIS DE IA"):
        with st.status("Llama-3.3 resolviendo tensores físicos..."):
            res = get_ai_analysis(f, t, p, b)
            if res: 
                st.session_state.data = res; st.session_state.screen = 'dash'; st.rerun()
            else:
                st.error("Error de comunicación con Groq. Reintente.")

elif st.session_state.screen == 'dash':
    d = st.session_state.data
    st.markdown(f"<h2 style='font-family:Syne; margin:20px;'>Command Center <span style='font-size:12px; color:#22D3EE; font-family:\"DM Mono\";'>[{d['label']}]</span></h2>", unsafe_allow_html=True)
    
    # Grid de KPIs principales
    k1, k2, k3, k4 = st.columns(4)
    with k1: st.markdown(f'<div class="kpi-box"><p class="kpi-label">AHORRO OPEX / AÑO</p><p class="kpi-value" style="color:#00E5A0;">${d["ahorro"]:,.0f}</p><p class="kpi-sub">≈ ${d["ahorro"]*20:,.0f} MXN</p></div>', unsafe_allow_html=True)
    ...
    with k4: st.markdown(f'<div class="kpi-box" style="border-top-color:#F59E0B;"><p class="kpi-label">CO2 EVITADO</p><p class="kpi-value" style="color:#F59E0B;">{d["co2"]:.0f}t</p></div>', unsafe_allow_html=True)

    # Gráfica y Métricas Duras
    ...
    with col_r:
        ...
        <div style="background:#0D1520; padding:25px; border-radius:12px; border:1px solid #1A2A3A;">
            <p style="font-size:10px; color:#64748B; font-weight:700;">DATOS DE INGENIERÍA</p>
            <p style="font-size:9px; color:#475569; margin-top:15px;">RESERVAS EUR EXTRA</p>
            <p style="font-family:'Syne'; font-size:26px; color:#22D3EE; margin:0;">{d['eur']:,.0f}</p>
            <p style="font-size:9px; color:#475569; margin-top:15px;">TASA CORROSIÓN</p>
            <p style="font-family:'Syne'; font-size:26px; color:{'#EF4444' if d['mpy'] > 5 else '#00E5A0'}; margin:0;">{d['mpy']:.1f} mpy</p>
        </div>
        """, unsafe_allow_html=True)
        
        pdf_b64 = generate_pdf_base64(d)
        st.markdown(f'<br><a href="data:application/pdf;base64,{pdf_b64}" download="FlowBio_Executive_Report.pdf" style="text-decoration:none;"><button style="background:#00E5A0; border:none; padding:15px; border-radius:8px; width:100%; color:#060B11; font-weight:800; cursor:pointer;">📥 DESCARGAR REPORTE PDF</button></a>', unsafe_allow_html=True)
        if st.button("← RE-CONFIGURAR"): st.session_state.screen = 'setup'; st.rerun()
