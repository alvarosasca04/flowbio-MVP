...
<style>
    ...
    .stButton > button { background: #00E5A0 !important; color: #060B11 !important; font-family: 'Syne' !important; font-weight: 800 !important; border-radius: 8px !important; padding: 15px 30px !important; }
</style>
...
def generate_corporate_pdf(well, d):
    ...
    for k, v in [["PV", f"{d.get('visc_p')} cP"], ["YP", f"{d.get('yield_p')} lb/ft2"], ["EUR", f"{d.get('eur'):,} bbls"]]:
        pdf.cell(95, 10, k, 1, 0, 'L', True)
        pdf.cell(95, 10, v, 1, 1, 'R')
...
if not st.session_state.auth:
    ...
    with cl:
        chart_html = f"""<script src="https://cdn.plot.ly/plotly-2.27.0.min.js"></script><div id="plot" style="height:400px; background:#0D1520; border-radius:12px;"></div>
        <script>
            ...
            Plotly.newPlot('plot', [{{x:x, y:y1, name:'Base', line:{{color:'#EF4444', dash:'dot'}}}},{{x:x, y:y2, name:'FlowBio AI', line:{{color:'#00E5A0', width:4}}, fill:'tonexty', fillcolor:'rgba(0,229,160,0.1)'}}], {{paper_bgcolor:'rgba(0,0,0,0)', plot_bgcolor:'rgba(0,0,0,0)', font:{{color:'#64748B'}}, margin:{{t:20, b:40, l:50, r:20}}}});
        </script>"""
    ...
    with cr:
        st.markdown(f"""<div style="background:#0D1520; padding:25px; border-radius:12px; border:1px solid rgba(0,229,160,0.3);">
            ...
            <p style="color:#64748B; font-size:10px;">INCREMENTAL: {d['eur']:,} bbls</p>
