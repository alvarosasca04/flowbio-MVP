...
with tab6:
    ...
    # Usar mensajes reales del JSON del Agente 5 si existen
    msg_perfil = rep_ag.get("mensajes_por_perfil", {})
    msg_eor = msg_perfil.get("eor_manager",
        f"Identificamos {res.get('candidatos_inyeccion',0)} pozos listos para inyección de Na-CMC. "
        f"Mejora +{res.get('mejora_promedio_pct',0):.1f}% eficiencia de barrido. Success Fee: $5/bbl.")
    msg_res = msg_perfil.get("reservoir_engineer",
        f"Motor PIML v0.3 · τ=K·γⁿ · Skin promedio: {res.get('skin_promedio',0):.2f}. "
        f"{res.get('pozos_criticos',0)} pozos con S>15.")
    msg_esg = msg_perfil.get("esg_manager",
        "Na-CMC FlowBio: 100% biodegradable, DNSH compliant. Economía circular.")

    pm1,pm2,pm3 = st.columns(3)
    perfiles_msg=[
        ("💼","EOR MANAGER","kcard-eor",BL, msg_eor),
        ("🔬","RESERVOIR ENGINEER","kcard-res",PU, msg_res),
        ("🌿","ESG MANAGER","kcard-esg",TEAL, msg_esg),
    ]
    ...
    # ── ESG / CBAM — nuevo: usar datos reales del Agente 4 ──
    esg_data = rep_ag.get("esg_cbam", {})
    if esg_data:
        st.markdown(f"<div class='sh'>🌿 ANÁLISIS ESG + CBAM · AGENTE 4</div>",
                    unsafe_allow_html=True)
        e1,e2,e3 = st.columns(3)
        for col,(v,c,l,t) in zip([e1,e2,e3],[
            (f"{esg_data.get('total_ton_co2_ahorradas',0):.1f} tCO₂","vt",
             "CO₂ AHORRADO/AÑO","kcard-esg"),
            (f"${esg_data.get('total_cbam_usd_evitado',0):,.0f}","vg",
             "CBAM EU EVITADO/AÑO","kcard-esg"),
            (f"{esg_data.get('pozos_esg_alto',0)}","vt",
             "POZOS ESG SCORE ALTO","kcard-esg"),
        ]):
            with col:
                st.markdown(f"""<div class='kcard {t}'>
                  <div class='klbl'>{l}</div>
    ...
    if rec_campo:
        ...
        st.markdown(f"""
        <div class='box-g'>
          <b style='color:{GR}'>📋 Recomendación de Campo · Agente 5</b><br><br>
          {rec_campo}
        </div>""", unsafe_allow_html=True)
