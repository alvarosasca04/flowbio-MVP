...
# ── LECTURA ESTRICTA DE LOS DATOS REALES ──
...
if not st.session_state.auth:
    ...
    with cr:
        ...
        diag_data = [
            ...
            ("Status Quo Promedio (P10)", f"{sum(r.get('P10',0) for r in proyeccion)/len(proyeccion):,.1f} BOPD" if proyeccion else "—"),
            ("Tratamiento Promedio (P50)", f"{sum(r.get('P50',0) for r in proyeccion)/len(proyeccion):,.1f} BOPD" if proyeccion else "—"),
